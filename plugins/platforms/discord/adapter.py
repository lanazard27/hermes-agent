...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # ── AI 팀 확장 모듈 로드 (git 추적 밖 — 업데이트 영향 없음) ────
        try:
            import sys as _sys
            _ext_dir = os.path.expanduser("~/.hermes/scripts/discord_team")
            if os.path.isdir(_ext_dir) and _ext_dir not in _sys.path:
                _sys.path.insert(0, _ext_dir)
            from team_extension import register_team_extension
            register_team_extension(self)
        except Exception:
            pass
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            async def on_ready():
                ...
                # AI 팀 확장: DiscordSyncManager 백그라운드 스레드 시작
                # (외부 모듈이 로드되어 있을 때만 동작)
                if hasattr(adapter_self, "_start_team_sync_manager"):
                    adapter_self._start_team_sync_manager()
            ...
            async def on_message(message: DiscordMessage):
                ...
                # AI 팀 확장: #order 채널 자동 실행 리스너.
                # 외부 모듈이 로드되어 있을 때만 동작. 매칭 시 return.
                if hasattr(adapter_self, "_try_handle_order_channel"):
                    if await adapter_self._try_handle_order_channel(message):
                        return
    ...
    def _register_slash_commands(self) -> None:
        ...
        # ── /chdel — channel purge (messages + threads) with bulk delete ──
        # Registered as a native command (not via the auto-register loop) so
        # the dedicated _handle_chdel_slash handler runs instead of the generic
        # _run_simple_slash proxy. Must be added BEFORE the auto-register
        # section so "chdel" lands in `already_registered` and is skipped there.
        @tree.command(name="chdel", description="Purge all messages and threads from a channel")
        @discord.app_commands.describe(
            channel="Channel name (general, dev, review) or numeric channel ID"
        )
        @discord.app_commands.choices(channel=[
            ...
            discord.app_commands.Choice(name="review", value="review"),
        ])
        async def slash_chdel(interaction: discord.Interaction, channel: str):
            await self._handle_chdel_slash(interaction, channel)

        logger.info("[Discord] Manually registered /chdel command (bulk-delete purge)")
    ...
    # Channel-name → ID map. config.yaml pipeline.channels is tried first
    # (so renaming channels in one place propagates); this hardcoded fallback
    # keeps /chdel working even when the config section is missing.
    _CHDEL_CHANNEL_MAP: Dict[str, int] = {
        "general": 1515592793387434025,
        "dev": 1515592794918490122,
        "review": 1515592796260663376,
    }

    def _resolve_chdel_target(self, channel: str) -> int | None:
        """Resolve a /chdel channel argument to a Discord channel ID.

        Accepts a known name (general/dev/review, case-insensitive), a raw
        numeric channel ID, or a pipeline.channels alias from config.yaml.
        Returns None when the argument matches nothing.
        """
        ...
        # 1) config.yaml pipeline.channels override
        try:
            pipeline = self.config.extra.get("pipeline", {}) or {}
            channels = pipeline.get("channels", {}) or {}
            if isinstance(channels, dict):
                for _name, _id in channels.items():
                    if str(_name).lower() == key:
                        try:
                            return int(_id)
                        except (TypeError, ValueError):
                            return None
        ...
        # 2) hardcoded map
        if key in self._CHDEL_CHANNEL_MAP:
            return self._CHDEL_CHANNEL_MAP[key]

        # 3) raw numeric channel ID
        if key.isdigit():
            return int(key)
        return None

    async def _handle_chdel_slash(
        self,
        interaction: discord.Interaction,
        channel: str,
    ) -> None:
        """Purge all messages and threads from a Discord channel.

        Uses bulk-delete (100/batch) for messages younger than 14 days and
        parallel individual deletes for older ones, then removes every thread
        (active, archived-public, archived-private). Reports counts via an
        ephemeral followup.
        """
        if not await self._check_slash_authorization(interaction, "/chdel"):
            return
        await interaction.response.defer(ephemeral=True)

        target_id = self._resolve_chdel_target(channel)
        if target_id is None:
            await interaction.followup.send(
                "❌ Unknown channel. Use general / dev / review, or a numeric "
                "channel ID.",
                ephemeral=True,
            )
            return
        ...
        try:
            result = await asyncio.wait_for(
                self._purge_discord_channel(target_id),
                timeout=600,
            )
        except asyncio.TimeoutError:
            await interaction.followup.send(
                f"⏱️ Purge of <#{target_id}> timed out after 10 minutes. "
                "Some content may already be deleted — re-run to finish.",
                ephemeral=True,
            )
            return
        except Exception as e:
            logger.exception("[Discord] /chdel purge failed for %s", target_id)
            await interaction.followup.send(
                f"❌ Purge failed: {e}", ephemeral=True,
            )
            return

        msgs = result["messages_deleted"]
        threads = result["threads_deleted"]
        await interaction.followup.send(
            f"🧹 <#{target_id}> purged — **{msgs}** message(s) and "
            f"**{threads}** thread(s) deleted.",
            ephemeral=True,
        )

    async def _purge_discord_channel(self, channel_id: int) -> Dict[str, int]:
        """Delete all messages + threads in a channel. Returns counts.

        Strategy:
          * threads first (archived public, archived private, then active) so
            thread messages don't reappear in the channel message listing.
          * messages: fetch 100 at a time, split by the 14-day bulk-delete
            cutoff (Discord API constraint). Recent batch → ``bulk_delete``;
            older batch → ``asyncio.gather`` of individual deletes (parallel
            within rate limits — discord.py handles 429 backoff per request).
        """
        client = self._client
        if client is None:
            return {"messages_deleted": 0, "threads_deleted": 0}
        ...
        if channel is None:
            try:
                ...
                logger.warning("[Discord] /chdel: channel %s not found: %s", channel_id, e)
                return {"messages_deleted": 0, "threads_deleted": 0}

        threads_deleted = 0

        # ── 1) Archived threads (public + private) ──
        for private in (False, True):
            try:
                async for thread in channel.archived_threads(limit=None, private=private):
                    try:
                        await thread.delete(reason="/chdel channel purge")
                        threads_deleted += 1
                    except discord.HTTPException as e:
                        if e.status == 429:
                            retry = getattr(e, "retry_after", None) or 2.0
                            ...
                            logger.debug("[Discord] /chdel thread delete fail: %s", e)
            except Exception as e:
                logger.debug("[Discord] /chdel archived_threads(%s) fail: %s", private, e)

        # ── 2) Active threads belonging to this channel ──
        try:
            if channel.guild is not None:
                active = channel.guild.threads
                for thread in list(active):
                    if getattr(thread, "parent_id", None) == channel_id:
                        try:
                            ...
                            if e.status == 429:
                                await asyncio.sleep(float(getattr(e, "retry_after", None) or 2.0))
                                with suppress(Exception):
                                    await thread.delete(reason="/chdel channel purge")
                                    threads_deleted += 1
        except Exception as e:
            logger.debug("[Discord] /chdel active threads fail: %s", e)

        # ── 3) Messages: bulk-delete recent, parallel-delete old ──
        messages_deleted = 0
        DISCORD_EPOCH_MS = 1420070400000
        FOURTEEN_DAYS_MS = 14 * 24 * 60 * 60 * 1000

        def _is_recent(msg_id: int) -> bool:
            ts = (msg_id >> 22) + DISCORD_EPOCH_MS
            return (int(time.time() * 1000) - ts) <= FOURTEEN_DAYS_MS

        while True:
            try:
                history = [m async for m in channel.history(limit=100, oldest_first=False)]
            except discord.HTTPException as e:
                if e.status == 429:
                    await asyncio.sleep(float(getattr(e, "retry_after", None) or 2.0))
                    continue
                logger.warning("[Discord] /chdel history fetch fail: %s", e)
                break

            if not history:
                break

            recent = [m for m in history if _is_recent(m.id)]
            old = [m for m in history if not _is_recent(m.id)]

            # Bulk delete recent (needs ≥2 messages). Leftover single recent
            # message gets folded into the old-delete path below.
            if len(recent) >= 2:
                try:
                    deleted = await channel.purge(
                        check=lambda m, _ids={m.id for m in recent}: m.id in _ids,
                        limit=len(recent),
                        bulk=True,
                    )
                    ...
                    if e.status == 429:
                        await asyncio.sleep(float(getattr(e, "retry_after", None) or 2.0))
                        continue
                    # Fallback: try explicit bulk_delete_messages
                    try:
                        await channel.delete_messages(recent[:100])
                        messages_deleted += len(recent[:100])
                    except Exception as e2:
                        logger.debug("[Discord] /chdel bulk fallback fail: %s", e2)
                        old = recent + old  # punt to individual deletes
            elif len(recent) == 1:
                old.append(recent[0])

            # Parallel individual deletes for old messages. discord.py's
            # rate-limit handler serializes 429 backoff per-route, so we can
            # fire many concurrent deletes safely.
            if old:
                sem = asyncio.Semaphore(5)

                async def _del_one(msg):
                    async with sem:
                        for _attempt in range(4):
                            try:
                                await msg.delete(reason="/chdel channel purge")
                                return True
                                ...
                                if e.status == 429:
                                    await asyncio.sleep(float(getattr(e, "retry_after", None) or 2.0))
                                    continue
                                logger.debug("[Discord] /chdel single delete fail: %s", e)
                                return False
                        return False

                results = await asyncio.gather(*[_del_one(m) for m in old], return_exceptions=False)
                messages_deleted += sum(1 for r in results if r)

            # If the channel had ≤100 messages total, we're done after one pass.
            if len(history) < 100:
                break

        logger.info(
            "[Discord] /chdel purge complete: channel=%s messages=%d threads=%d",
            channel_id, messages_deleted, threads_deleted,
        )
        return {"messages_deleted": messages_deleted, "threads_deleted": threads_deleted}

    async def purge_channel_by_name(self, channel_key: str) -> str:
        """Text-command entry point for channel purge.

        Mirrors :meth:`_handle_chdel_slash` but is driven by a plain string
        argument (typed ``/chdel <channel>``) instead of a Discord
        interaction, so the gateway can route text-input ``/chdel`` to it
        without Discord's slash-command menu. Reuses
        :meth:`_resolve_chdel_target` and :meth:`_purge_discord_channel` —
        no purge logic is duplicated.

        Returns a human-readable result message (success or error).
        """
        raw = (channel_key or "").strip()
        if not raw:
            return (
                "❌ Usage: `/chdel <channel>` — use general / dev / review, "
                "or a numeric channel ID."
            )

        target_id = self._resolve_chdel_target(raw)
        if target_id is None:
            return (
                "❌ Unknown channel. Use general / dev / review, or a numeric "
                "channel ID."
        ...
        try:
            ...
            logger.exception("[Discord] /chdel (text) purge failed for %s", target_id)
