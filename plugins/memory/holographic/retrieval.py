...
class FactRetriever:
    ...
    def _sanitize_fts_query(cls, query: str) -> str:
        ...
        for raw in query.lower().split():
            ...
            # CJK(Chinese/Japanese/Korean) characters are single-syllable
            # words (툴, 앱, 맵) that carry full meaning. Latin <2 chars
            # (s, a, q) are noise, but CJK 1-char tokens are valid search
            # terms. Accept any token that is either ≥2 chars OR contains
                ...
                for ch in s:
                    ...
                    for lo, hi in _CJK_RANGES:
                        if lo <= cp <= hi:
                            return True
                return False
            if len(cleaned) < 2 and not _has_cjk(cleaned):
                continue
